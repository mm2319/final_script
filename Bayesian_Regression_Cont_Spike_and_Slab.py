...
def Bayesian_regression_conti_spike_slab(Y_1, X_1, size_fun_lib):
    ...
    with basic_model:
        ...
        sigma_hat = 10
        tau = 5
        lambda_hat = pm.Normal('lambda_hat', mu = mu_hat, sd = sigma_hat, shape = size_fun_lib)
        spike_raw = pm.Normal('spike_raw', mu = 0, sd = 1, shape = size_fun_lib)
    ...
    with basic_model: 
        trace_rh = pm.sample(2500, tune=2500, cores=1, random_seed=1, nuts={'target_accept':0.9})
    with basic_model:
        start = pm.find_MAP()
